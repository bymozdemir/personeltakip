    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      topluWhatsappButonunuEnjekteEt();
    });
    ...
    function topluWhatsappButonunuEnjekteEt() {
      const actionContainer = document.querySelector('.action-buttons') || document.getElementById('viewMain');
      if (actionContainer && !document.getElementById('topluWhatsappBtn')) {
        let btn = document.createElement('button');
        btn.id = 'topluWhatsappBtn';
        btn.innerHTML = '💬 Toplu WhatsApp Duyurusu';
        btn.style.cssText = `
          background: #25d366; color: white; border: none; padding: 12px 16px;
          border-radius: 8px; font-weight: bold; cursor: pointer; 
          margin: 12px 0 15px 0; width: 100%; box-sizing: border-box; font-size: 14px;
          box-shadow: 0 2px 8px rgba(37, 211, 102, 0.3);
        `;
        btn.onclick = topluWhatsappModalAc;
    ...
    function topluWhatsappModalAc() {
      if (seciliPersonelIds.size === 0) {
        return özelBildirimGoster("Lütfen toplu duyuru göndermek için en az bir personel seçin!");
      }

      let hedefKisiler = [];
      seciliPersonelIds.forEach(id => {
        ...
        if (p) {
          ...
          if (tel) {
            ...
            hedefKisiler.push({ isim: p.adSoyad, tel: temizTel });
          }
        }
      });

      if (hedefKisiler.length === 0) {
        return özelBildirimGoster("Seçilen personeller arasında kayıtlı şahsi telefon numarası bulunan kimse yok!");
      }
      ...
      let listeHtml = hedefKisiler.map(k => `<div style="font-size:12px; padding:4px 0; border-bottom:1px solid #eee; display:flex; justify-content:space-between;"><span>✓ ${k.isim}</span><span style="color:#666;">${k.tel}</span></div>`).join('');

      modal.innerHTML = `
        <div style="background:white; width:90%; max-width:420px; max-height:85vh; overflow-y:auto; padding:20px; border-radius:14px; box-shadow:0 4px 20px rgba(0,0,0,0.25); text-align:left;">
          <h3 style="margin-top:0; color:#25d366; font-size:16px;">💬 Toplu WhatsApp Duyurusu</h3>
          <div style="font-size:12px; color:#666; margin-bottom:8px;">Seçilen ve şahsi numarası olan <b>${hedefKisiler.length}</b> kişiye gönderilecek ortak mesajı yazın:</div>
          <textarea id="topluMesajInput" placeholder="Duyuru / mesaj metnini buraya yazın..." style="width:100%; height:90px; padding:10px; border:1px solid #ccc; border-radius:8px; font-size:13px; box-sizing:border-box; resize:vertical;"></textarea>
          
          <div style="font-size:12px; color:#444; margin-top:12px; font-weight:bold;">Alıcı Listesi (${hedefKisiler.length} Kişi):</div>
          <div style="max-height:130px; overflow-y:auto; border:1px solid #eee; background:#fafafa; padding:8px; border-radius:8px; margin-top:4px; margin-bottom:15px;">
            ${listeHtml}
          </div>

          <div style="display:flex; justify-content:flex-end; gap:10px;">
            <button id="topluIptalBtn" style="background:#999; color:white; border:none; padding:8px 14px; border-radius:8px; font-weight:bold; cursor:pointer;">İptal</button>
            <button id="topluGonderBtn" style="background:#25d366; color:white; border:none; padding:8px 14px; border-radius:8px; font-weight:bold; cursor:pointer;">WhatsApp Paneli Aç</button>
          </div>
        </div>
      `;
      document.body.appendChild(modal);

      modal.querySelector('#topluIptalBtn').onclick = () => modal.remove();
      modal.querySelector('#topluGonderBtn').onclick = () => {
        let mesajMetni = modal.querySelector('#topluMesajInput').value.trim();
        if (!mesajMetni) {
          özelBildirimGoster("Lütfen gönderilecek mesajı yazın!");
          return;
        }
        modal.remove();
        topluWhatsappPanelGoster(hedefKisiler, mesajMetni);
      };
    }

    function topluWhatsappPanelGoster(hedefKisiler, mesajMetni) {
      const modal = document.createElement('div');
      modal.style.cssText = `
        position: fixed; top: 0; left: 0; width: 100%; height: 100%;
        background: rgba(0,0,0,0.5); display: flex; align-items: center;
      ...
      let encodedMsg = encodeURIComponent(mesajMetni);
      let satirHtml = hedefKisiler.map((k, index) => `
        <div style="display:flex; align-items:center; justify-content:space-between; padding:8px; border-bottom:1px solid #eee; background:${index % 2 === 0 ? '#fff' : '#fafafa'}; border-radius:6px; margin-bottom:4px;">
          <div>
            <div style="font-weight:bold; font-size:13px; color:#333;">${index + 1}. ${k.isim}</div>
            <div style="font-size:11px; color:#666;">${k.tel}</div>
          </div>
          <a href="https://wa.me/${k.tel}?text=${encodedMsg}" target="_blank" onclick="this.style.background='#81c784'" style="background:#25d366; color:white; padding:6px 12px; border-radius:6px; text-decoration:none; font-size:12px; font-weight:bold; display:inline-block;">💬 Mesaj Gönder</a>
        </div>
      `).join('');

      modal.innerHTML = `
        <div style="background:white; width:92%; max-width:450px; max-height:85vh; overflow-y:auto; padding:20px; border-radius:14px; box-shadow:0 4px 20px rgba(0,0,0,0.25); text-align:left;">
          <h3 style="margin-top:0; color:#25d366; font-size:16px;">🚀 WhatsApp Gönderim Paneli</h3>
          <div style="font-size:12px; color:#666; margin-bottom:12px;">Tarayıcı engelini aşmak için aşağıdaki butonlara sırayla tıklayarak her personelin WhatsApp sohbetini tek tıkla açabilirsiniz:</div>
          
          <div style="max-height:300px; overflow-y:auto; border:1px solid #ddd; padding:8px; border-radius:8px; background:#f5f5f5; margin-bottom:15px;">
            ${satirHtml}
          </div>
